...
async function generateAIHealthAnalysis(data: HealthData): Promise<AIHealthAnalysis> {
...
- Work: ${data.lifestyle.workSchedule.start} - ${data.lifestyle.workSchedule.end}
- Meals: Breakfast: ${data.lifestyle.mealTimes.breakfast}, Lunch: ${data.lifestyle.mealTimes.lunch}, Dinner: ${data.lifestyle.mealTimes.dinner}
    ...
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are a professional health analyst. Provide accurate, helpful health insights based on user data. Always respond with valid JSON only.'
          },
          {
            role: 'user',
            content: prompt
...
function generateFallbackAnalysis(data: HealthData): AIHealthAnalysis {
  ...
  // Activity Level
  const hasActiveGoals = data.health.healthGoals?.some(goal => 
  ...
  const hasHealthyDiet = data.health.dietType && 
    ['vegetarian', 'vegan', 'mediterranean', 'balanced'].includes(data.health.dietType.toLowerCase());
