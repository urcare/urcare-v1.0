import { HealthNotification, notificationService } from "./notificationService";
// AI Health Assistant service removed
...
export class HealthNotificationScheduler {
  ...
  static async schedulePlanNotifications(
    ...
    try {
      ...
      const medicationNotifications =
        await this.scheduleMedicationNotifications(userId, plan);
      notifications.push(...medicationNotifications);

      // Schedule hydration reminders
      const hydrationNotifications = await this.scheduleHydrationNotifications(
        userId,
        plan
      );
      ...
      const exerciseNotifications = await this.scheduleExerciseNotifications(
        userId,
        plan
      );
      ...
      const mealNotifications = await this.scheduleMealNotifications(
        userId,
        plan
      );
      ...
      const lifestyleNotifications = await this.scheduleLifestyleNotifications(
        userId,
        plan
      );
      ...
      console.log(
        `Scheduled ${notifications.length} notifications for user ${userId}`
      );
      return schedule;
    } catch (error) {
  ...
  private static async createEventNotification(
    ...
    try {
      ...
      const scheduledTime = new Date(
        eventTime.getTime() - advanceNotice * 60 * 1000
      );
  ...
  private static async scheduleHydrationNotifications(
    ...
    try {
      ...
      const interval =
        (endTime.getTime() - startTime.getTime()) / remindersPerDay;
      ...
      for (let i = 0; i < remindersPerDay; i++) {
        ...
        > = {
          ...
          message: `Time to hydrate! Aim for ${Math.round(
            hydrationTarget / remindersPerDay
          )}ml of water.`,
    ...
    try {
      ...
      for (const exercise of plan.exerciseSchedule) {
        ...
        > = {
          ...
          message: `Get ready for ${exercise.type}! Duration: ${
            exercise.duration
          } minutes. ${exercise.instructions || ""}`,
          scheduledTime,
    ...
    try {
      ...
      for (const meal of plan.nutritionSchedule) {
        ...
        > = {
          ...
          message: `Time for ${meal.type}! Target: ${
            meal.targetCalories
          } calories. ${meal.instructions || ""}`,
          scheduledTime,
          eventTime: meal.time,
          category: meal.type,
          priority: "medium",
   ...
   */
  private static mapEventTypeToNotificationType(
    eventType: string
  ): HealthNotification["type"] {
  ...
  private static generateNotificationMessage(event: any): string {
    ...
    const calories = event.calories
      ? `Target: ${event.calories} calories. `
      : "";
  ...
  private static determinePriority(event: any): HealthNotification["priority"] {
    ...
    if (
      ["breakfast", "lunch", "dinner", "cardio", "strength"].includes(
        event.type
      )
    ) {
      return "medium";
  ...
  private static isActionRequired(event: any): boolean {
    ...
    if (["breakfast", "lunch", "dinner", "snack"].includes(event.type))
      return false;
   ...
   */
  static async cancelPlanNotifications(
    userId: string,
    planId: string
  ): Promise<void> {
    try {
