...
const Onboarding = () => {
  ...
  const validateOnboardingData = (
    ...
    if (!data.heightCm || parseInt(data.heightCm) <= 0)
      errors.push("Valid height is required");
    if (!data.weightKg || parseInt(data.weightKg) <= 0)
    ...
    if (
      data.hasSurgery === "Yes" &&
      (!data.surgeryDetails || data.surgeryDetails.length === 0)
    ) {
  ...
  const generateDataSummary = (data: OnboardingData) => {
    const categories = {
      ...
      "Health Metrics": [data.heightCm, data.weightKg, data.bloodGroup].filter(
        Boolean
      ).length,
      "Schedule & Lifestyle": [
        data.wakeUpTime,
      ...
      "Health Information": [
        ...
        data.medications?.length > 0 ? "meds" : "",
        data.hasSurgery,
        data.healthGoals?.length > 0 ? "goals" : "",
      ].filter(Boolean).length,
      "Emergency & Contact": [
  ...
  const handleSerialComplete = useCallback(
    async (data: OnboardingData) => {
      ...
      try {
        ...
        const age =
          parseInt(data.age) ||
          new Date().getFullYear() - parseInt(data.birthYear);
        ...
        const date_of_birth = `${data.birthYear}-${monthIndex
          .toString()
          .padStart(2, "0")}-${data.birthDay.padStart(2, "0")}`;
        ...
        const preferences = {
          ...
          health: {
            ...
            surgery_details: Array.isArray(data.surgeryDetails)
              ? data.surgeryDetails
              : [],
            critical_conditions: data.criticalConditions,
            health_goals: Array.isArray(data.healthGoals)
              ? data.healthGoals
              : [],
          },
        };
        ...
        const cleanUserProfileData = {
          ...
          health_reports: Array.isArray(data.healthReports)
            ? data.healthReports
            : [],
        ...
        const { error, data: upsertData } = (await Promise.race([
          upsertPromise,
          timeoutPromise,
        ])) as {
          error: {
        ...
        if (error) {
          ...
          if (insertError) {
            ...
            if (error.code === "PGRST116") {
              ...
              errorMessage =
                "Profile already exists. Updating existing profile...";
            } else if (error.message?.includes("network")) {
              errorMessage =
                "Network error. Please check your connection and try again.";
            } else if (error.message?.includes("validation")) {
              errorMessage =
                "Data validation error. Please check your information.";
            }
