...
export async function createPhonePeTransaction(
  ...
  try {
    ...
    const payload = {
      ...
      amount: amount,
      redirectUrl: request.redirectUrl || `${process.env.VERCEL_URL || 'https://urcare.org'}/api/phonepe/callback`,
      redirectMode: 'POST',
      callbackUrl: `${process.env.VERCEL_URL || 'https://urcare.org'}/api/phonepe/notify`,
