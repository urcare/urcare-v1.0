...
export const AuthOptions: React.FC<AuthOptionsProps> = ({ onboardingData, onAuthSuccess }) => {
  ...
  const handleAppleSignIn = async () => { await signInWithApple(); onAuthSuccess(); };
  const handleGoogleSignIn = async () => { await signInWithGoogle(); onAuthSuccess(); };
  const handleEmailSignIn = async () => { await signInWithEmail(); onAuthSuccess(); };
