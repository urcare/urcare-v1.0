...
const PHONEPE_CONFIG = {
  ...
  // Set to true when PhonePe account is activated
  accountActivated: process.env.PHONEPE_ACTIVATED === 'true' || false
};
...
app.post('/api/phonepe/pay', async (req, res) => {
  try {
    ...
    // If account is not activated, use realistic mock
    if (!PHONEPE_CONFIG.accountActivated) {
      console.log('🎭 Account not activated, using realistic mock');
      
      // Simulate processing delay
      await new Promise(resolve => setTimeout(resolve, 1500));

      // Create a realistic PhonePe payment URL that looks like the real thing
      const mockRedirectUrl = `https://mercury.phonepe.com/transact?token=${orderId}&merchantId=${PHONEPE_CONFIG.merchantId}&amount=${amountInPaise}&redirectUrl=${encodeURIComponent(`${PHONEPE_CONFIG.frontendUrl}/payment/success?orderId=${orderId}&plan=${planSlug || 'basic'}&cycle=${billingCycle || 'annual'}&success=true`)}`;
    ...
    // Real PhonePe integration (when account is activated)
