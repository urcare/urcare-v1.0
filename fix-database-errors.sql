-- FIX DATABASE ERRORS
-- This script fixes the 406 and 400 errors
...
-- Create policy for health_scores if it doesn't exist
DO $$ 
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policy 
        WHERE policyname = 'Allow all for health_scores' 
...
-- Create policy for daily_activities if it doesn't exist
DO $$ 
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policy 
        WHERE policyname = 'Allow all for daily_activities' 
